  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!(await canPropose(user))) {
        return res
          .status(403)
          .json({ error: "Unauthorized to create proposals." });
      }

      if (
        await cuffIfSpam(user.address, proposal.title, proposal.description)
      ) {
      ...
      if (!topic || topic.proposalId) {
        return res
          ...
          .json({
            error:
              "Invalid topicId: not found or already linked to a proposal.",
          });
      }
    ...
    try {
      ...
      if (!(await isModerator(user.address))) {
        if (proposal.status !== "pending") {
          return res
            .status(400)
            .json({ error: "Proposal can only be modified while pending." });
        }
        if (!(await canEdit(user, proposal))) {
          return res
            .status(403)
            .json({ error: "User is not eligible to edit this proposal." });
        }
      }
      await pushProposal(proposal);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!(await isModerator(user.address))) {
        return res
          .status(403)
          .json({ error: "Unauthorized to change proposal status." });
      }
      ...
      if (
        ["closed", "cancelled"].includes(proposal.status) &&
        status === "active"
      ) {
        return res
          .status(400)
          .json({
            error: "Cannot re-activate a closed or cancelled proposal.",
          });
      }
      ...
      const updatedProposal: Proposal = {
        ...
        status,
        startDate:
          status === "active" && !proposal.startDate ? now : proposal.startDate,
...
router.delete("/:proposalId", useAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!(await isModerator(user.address))) {
      return res
        .status(403)
        .json({ error: "Unauthorized to delete proposal." });
    }
